...
enum class ConversationType {
    ...
    companion object {
        /** Optional: parse an toàn từ chuỗi (nếu cần) */
        fun from(raw: String?): ConversationType = when (raw?.lowercase()) {
            "private" -> PRIVATE
            "group"   -> GROUP
            else      -> GROUP
        }
...
data class ConversationPublic(
    @SerializedName("name")             val name: String?,
    @SerializedName("type")             val type: ConversationType = ConversationType.PRIVATE,
    @SerializedName("id")               val id: String,
    @SerializedName("last_message_id")  val lastMessageId: String?,
    @SerializedName("members")          val members: List<ConversationMember> = emptyList(),
